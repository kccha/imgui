...
{
	bool result = DragScalar(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format, power);
	return result;
}

bool ImGui::DragFloat2(const char* label, float v[2], float v_speed, float v_min, float v_max, const char* format, float power)
{
	bool result = DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed, &v_min, &v_max, format, power);
	return result;
}
